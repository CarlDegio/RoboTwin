...
from .math_utils import make_matrix_from_quaternion, quaternion_inv, quaternion_mul, make_quat_from_rpy, make_rpy_from_matrix, make_matrix_from_rpy
...
class pika_arm:
    def __init__(self, arm_port="can0", gripper_port="/dev/ttyUSB0", camera_param=(640, 480, 30, 100), fisheye_camera_index=0, realsense_serial_number='230322275842'):
        ...
        self.gripper_offset = np.array([0.0, 0.0, 190.34]) # mm
        ...
        self.smooth_buffer = []
        self.smooth_weight = np.array([0.2, 0.3, 0.5])
    ...
    def get_arm_pose(self):
        ...
        """
        arm_end_pose_msg = self.piper_arm.GetArmEndPoseMsgs()
        arm_end_pose = arm_end_pose_msg.end_pose
        x = arm_end_pose.X_axis/self.scale_factor
        y = arm_end_pose.Y_axis/self.scale_factor
        z = arm_end_pose.Z_axis/self.scale_factor
        rx = arm_end_pose.RX_axis/self.scale_factor
        ry = arm_end_pose.RY_axis/self.scale_factor
        rz = arm_end_pose.RZ_axis/self.scale_factor
        position = np.array([x, y, z])
        quat = make_quat_from_rpy(np.deg2rad(rx), np.deg2rad(ry), np.deg2rad(rz))
        rpy = np.array([rx, ry, rz])
        return position, quat, rpy
    ...
    def get_arm_pose_offset(self):
        ...
        z = arm_end_pose.Z_axis/self.scale_factor
        quat = make_quat_from_rpy(np.deg2rad(rx), np.deg2rad(ry), np.deg2rad(rz))
        rotation_matrix = make_matrix_from_rpy(np.deg2rad(rx), np.deg2rad(ry), np.deg2rad(rz))
        
        position = np.matmul(rotation_matrix, self.gripper_offset) + np.array([x, y, z])
    ...
    def control_arm_joint(self, joint1, joint2, joint3, joint4, joint5, joint6):
        ...
        self.piper_arm.MotionCtrl_2(0x01, 0x01, 50, 0x00)
        self.piper_arm.JointCtrl(joint1, joint2, joint3, joint4, joint5, joint6)
        
    
    def control_arm_end_pose(self, x, y, z, rx, ry, rz):
        # 毫米与度
        x = x * self.scale_factor
        y = y * self.scale_factor
        z = z * self.scale_factor
        rx = rx * self.scale_factor
        ry = ry * self.scale_factor
        rz = rz * self.scale_factor
        x, y, z, rx, ry, rz = int(x), int(y), int(z), int(rx), int(ry), int(rz)
        self.piper_arm.MotionCtrl_2(0x01, 0x00, 50, 0x00)
    ...
    def control_arm_gripper_end_pose(self, x, y, z, rx, ry, rz, gripper_rad):    
        self.control_arm_end_pose(x, y, z, rx, ry, rz)
    ...
    def control_by_tele(self, tele_position, tele_rotation_quaternion, gripper_rad):
        ...
        command_rpy = make_rpy_from_matrix(command_rotation_matrix)
        ...
        command_rpy = np.rad2deg(command_rpy)
        
        # print("command_quat", command_quat)
        # print("command_position", command_position)
        self.control_arm_end_pose(command_position[0], command_position[1], command_position[2], command_rpy[0], command_rpy[1], command_rpy[2])
        self.control_gripper(gripper_rad)
        
        return command_position, command_quat, gripper_rad, command_rpy
...
class pika_sense:
    ...
    def on_record_start(self):
        self.abs_pose0 = self.get_vive_abs_pose()
